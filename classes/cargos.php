...
/**
 * Table class for cargos
 */
class cargos extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'cargos';
		$this->TableName = 'cargos';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "\"cargos\"";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// codigo
		$this->codigo = new DbField('cargos', 'cargos', 'x_codigo', 'codigo', '"codigo"', 'CAST("codigo" AS varchar(255))', 3, -1, FALSE, '"codigo"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// descricao
		$this->descricao = new DbField('cargos', 'cargos', 'x_descricao', 'descricao', '"descricao"', '"descricao"', 200, -1, FALSE, '"descricao"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "\"cargos\"";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->codigo->setDbValue($conn->getOne("SELECT currval('cargos_codigo_seq'::regclass)"));
	...
	{
		global $Language;
		if ($pageName == "cargosview.php")
			return $Language->phrase("View");
		elseif ($pageName == "cargosedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "cargosadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("cargosview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("cargosview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "cargosadd.php?" . $this->getUrlParm($parm);
		else
			$url = "cargosadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("cargosedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("cargosadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("cargosdelete.php", $this->getUrlParm());
	}
