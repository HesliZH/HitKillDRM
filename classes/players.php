...
/**
 * Table class for players
 */
class players extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		// codigo
		$this->codigo = new DbField('players', 'players', 'x_codigo', 'codigo', '"codigo"', 'CAST("codigo" AS varchar(255))', 3, -1, FALSE, '"codigo"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// nome_completo
		$this->nome_completo = new DbField('players', 'players', 'x_nome_completo', 'nome_completo', '"nome_completo"', '"nome_completo"', 200, -1, FALSE, '"nome_completo"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nome_completo->Sortable = TRUE; // Allow sort
		$this->fields['nome_completo'] = &$this->nome_completo;

		// usuario
		$this->usuario = new DbField('players', 'players', 'x_usuario', 'usuario', '"usuario"', '"usuario"', 200, -1, FALSE, '"usuario"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->usuario->Sortable = TRUE; // Allow sort
		$this->fields['usuario'] = &$this->usuario;

		// senha
		$this->senha = new DbField('players', 'players', 'x_senha', 'senha', '"senha"', '"senha"', 200, -1, FALSE, '"senha"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->senha->Sortable = TRUE; // Allow sort
		$this->fields['senha'] = &$this->senha;

		// nickname
		$this->nickname = new DbField('players', 'players', 'x_nickname', 'nickname', '"nickname"', '"nickname"', 200, -1, FALSE, '"nickname"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "\"players\"";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->codigo->setDbValue($conn->getOne("SELECT currval('players_codigo_seq'::regclass)"));
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->nome_completo->DbValue = $row['nome_completo'];
	...
	{
		global $Language;
		if ($pageName == "playersview.php")
			return $Language->phrase("View");
		elseif ($pageName == "playersedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "playersadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("playersview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("playersview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "playersadd.php?" . $this->getUrlParm($parm);
		else
			$url = "playersadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("playersedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("playersadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("playersdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->nome_completo->setDbValue($rs->fields('nome_completo'));
	...
	public function renderEditRow()
	{
		...
		$this->nome_completo->EditValue = $this->nome_completo->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->nome_completo);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->nome_completo);
