...
class usuarios extends DbTable
{
	...
	public function __construct()
	{
		...
		// codigo
		$this->codigo = new DbField('usuarios', 'usuarios', 'x_codigo', 'codigo', '"codigo"', 'CAST("codigo" AS varchar(255))', 3, -1, FALSE, '"codigo"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// nome_completo
		$this->nome_completo = new DbField('usuarios', 'usuarios', 'x_nome_completo', 'nome_completo', '"nome_completo"', '"nome_completo"', 200, -1, FALSE, '"nome_completo"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nome_completo->Sortable = TRUE; // Allow sort
		$this->fields['nome_completo'] = &$this->nome_completo;

		// usuario
		$this->usuario = new DbField('usuarios', 'usuarios', 'x_usuario', 'usuario', '"usuario"', '"usuario"', 200, -1, FALSE, '"usuario"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->usuario->Required = TRUE; // Required field
		$this->usuario->Sortable = TRUE; // Allow sort
		$this->fields['usuario'] = &$this->usuario;

		// senha
		$this->senha = new DbField('usuarios', 'usuarios', 'x_senha', 'senha', '"senha"', '"senha"', 200, -1, FALSE, '"senha"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->senha->Required = TRUE; // Required field
		$this->senha->Sortable = TRUE; // Allow sort
		$this->fields['senha'] = &$this->senha;

		// cargo
		$this->cargo = new DbField('usuarios', 'usuarios', 'x_cargo', 'cargo', '"cargo"', 'CAST("cargo" AS varchar(255))', 3, -1, FALSE, '"cargo"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		switch ($CurrentLanguage) {
			case "en":
				$this->cargo->Lookup = new Lookup('cargo', 'cargos', FALSE, 'codigo', ["descricao","","",""], [], [], [], [], [], [], '', '');
				break;
			default:
				$this->cargo->Lookup = new Lookup('cargo', 'cargos', FALSE, 'codigo', ["descricao","","",""], [], [], [], [], [], [], '', '');
				break;
		}
		$this->cargo->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
	...
	protected function insertSql(&$rs)
	{
		...
		foreach ($rs as $name => $value) {
			if (!isset($this->fields[$name]) || $this->fields[$name]->IsCustom)
				continue;
			if (ENCRYPTED_PASSWORD && $name == 'senha')
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->codigo->setDbValue($conn->getOne("SELECT currval('usuarios_codigo_seq'::regclass)"));
	...
	public function getViewUrl($parm = "")
	{
		if ($parm <> "")
			...
			$url = $this->keyUrl("usuariosview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
