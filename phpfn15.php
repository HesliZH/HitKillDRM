...
function CurrentProjectID() {
	...
	return "{D25E8543-1442-438F-944C-0B1439EAA2B1}";
}
	...
	public function validateUser(&$usr, &$pwd, $autologin, $provider = "")
	{
		...
		if (!$valid) {
			...
			if ($rs = $UserTableConn->execute($sql)) {
				if (!$rs->EOF) {
					$valid = $customValid || ComparePassword($rs->fields('senha'), $pwd);
					if ($valid) {
						$this->_isLoggedIn = TRUE;
						$_SESSION[SESSION_STATUS] = "login";
						...
						$UserProfile->delete('senha'); // Delete password
...
class Language
{
	...
	public function __construct($langFolder = "", $langId = "")
	{
		...
		global $CSS_FLIP, $RTL_LANGUAGES;
		$CSS_FLIP = in_array($CurrentLanguage, $RTL_LANGUAGES);
