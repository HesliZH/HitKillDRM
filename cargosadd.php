...
var fcargosadd = currentForm = new ew.Form("fcargosadd", "add");
...
fcargosadd.validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($cargos_add->descricao->Required) { ?>
...
fcargosadd.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;
...
<?php if ($cargos_add->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $cargos_add->Token ?>">
<?php } ?>
...
<?php if ($cargos->descricao->Visible) { // descricao ?>
	<div id="r_descricao" class="form-group row">
		<label id="elh_cargos_descricao" for="x_descricao" class="<?php echo $cargos_add->LeftColumnClass ?>"><?php echo $cargos->descricao->caption() ?><?php echo ($cargos->descricao->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $cargos_add->RightColumnClass ?>"><div<?php echo $cargos->descricao->cellAttributes() ?>>
